...
try:
    ...
    # Completely remove the debug information from UI
    # The debug information was previously shown here but has been removed per user request
    ...
    # Add a more robust initialization section
    ...
    backend_available = initialize_application()

    # After backend connection test, update the status
    # Add a clear indication if backend is not available
    ...
    with tab1:
        ...
        if backend_connected:
            ...
            with st.form(key="chat_form", clear_on_submit=True):
                ...
                # Add checkbox to enable/disable tools
                use_tools = st.checkbox("Enable AI Tools", value=True, 
                                      help="Enable AI tools for enhanced responses (e.g., code generation, data analysis)")
                        ...
                        with st.chat_message("assistant"):
                            with st.spinner("Thinking..."):
                                success, response_data, error_msg = api_client.send_chat_message(
                                    ...
                                    use_tools=use_tools,  # Use the checkbox value
            ...
            if cached_tasks:
                ...
                for task in sorted_tasks:
                    ...
                    with st.expander(task_display):
                        ...
                        # Add status update controls for all tasks regardless of current status
                        current_status = task.get('status', 'Not Started')
                        # Show status update controls for all actionable statuses
                        if current_status in ['In Progress', 'Completed', 'Failed']:
                            st.divider()
                            st.subheader("Update Task Status")
                            
                            # Status selection - allow switching to any status
                            all_status_options = ["In Progress", "Completed", "Failed"]
                            
                            # Determine default selection index
                            if current_status in all_status_options:
                                default_index = all_status_options.index(current_status)
                            else:
                                default_index = 0
                            ...
                            # Set default progress based on selected status
                            if new_status == 'Completed':
                                default_progress = 100
                            elif new_status == 'In Progress':
                                # If coming from Completed, start at 90%, otherwise maintain current progress
                                default_progress = 90 if current_status == 'Completed' else (current_progress if current_progress < 100 else 50)
                            else:  # Failed
                                default_progress = current_progress
                            
                            new_progress = st.slider(
                                "Progress (%)",
                                0, 100, default_progress,
                                key=f"progress_slider_{task['id']}"
                            )
                            
                            # Update button
                            if st.button("Update Status", key=f"update_btn_{task['id']}"):
                                with st.spinner("Updating task..."):
                                    success, error_msg = api_client.update_task(
                                        task_id=str(task['id']),
                                        status=new_status,
                                    ...
                                    if success:
                                        st.success(f"Task updated to {new_status} with {new_progress}% progress!")
                                        # Clear cache to force refresh
                                        clear_cache(cache_type="tasks")
                                        # Rerun to show updated status
                                        st.rerun()
                                    else:
                                        st.error(f"Failed to update task: {error_msg or 'Unknown error'}")
                        elif current_status == 'Not Started':
                            # For Not Started tasks, provide option to move to In Progress
                            st.divider()
                            if st.button("Start Task", key=f"start_btn_{task['id']}"):
                                with st.spinner("Starting task..."):
                                    ...
                                    if success:
                                        ...
                                        st.error(f"Failed to start task: {error_msg or 'Unknown error'}")
    ...
    st.set_page_config(page_title="AI Workbench - Error", layout="centered")
    st.title("🚨 AI Workbench - Initialization Error")
    st.error(f"Application failed to start due to an error: {str(e)}")
    ...
    4. Check the application logs for specific error details
    
    ### Required Environment Variables:
    - `BACKEND_API_KEY`: Authentication key for backend access
    - `OPENAI_API_KEY`: OpenAI API key for AI functionality
    - `SUPABASE_URL` and `SUPABASE_KEY`: Database connection details
    """)
